const resources = [
    ...
    { category: "claude-code-skills", title: "NotebookLM Context Skill", provider: "Community", desc: "Reduce token burn by 90% and fix Claude's project amnesia. This skill uses NotebookLM to store your project docs locally and lets Claude query only relevant pieces for ~200 tokens instead of 8,000.", type: "Guide", url: "#",
      why: "Every new Claude Code session starts with zero memory of your project. Loading every doc into context wastes thousands of tokens per turn. NotebookLM acts as an external project brain that Claude can query on demand, ensuring it uses the right conventions without guessing.",
      how: "1. `pip install notebooklm` (and fix PATH if needed). 2. `notebooklm create my-project`. 3. Add sources: `notebooklm add my-project --url/--pdf/--text`. 4. `notebooklm index my-project`. 5. Tell Claude: 'Create a NotebookLM skill at .claude/skills/notebooklm/SKILL.md that runs notebooklm query my-project before architecture tasks.' Add the same rule to your CLAUDE.md." },
    { category: "claude-code-skills", title: "Playwright Web Scraping Skill", provider: "Community", desc: "Give Claude Code a browser. This skill lets Claude control headless Chromium to scrape pages, fill forms, and extract clean JSON data for 1/100th the cost of using screenshots.", type: "Guide", url: "#",
      why: "MCP screenshots are slow and extremely token-expensive (2,000+ tokens per page). Playwright returns clean JSON, allowing Claude to extract data from 100+ pages for the cost of one screenshot. It makes form filling and table extraction fast and reliable.",
      how: "1. `cd your-project`. 2. Ask Claude: 'Set up a Playwright skill at .claude/skills/playwright/ with a SKILL.md and scripts/run.js that returns page title, text, and links as JSON.' 3. `npm install playwright && npx playwright install chromium`. 4. Add 'Browser Automation' section to CLAUDE.md. Now you can just say 'Scrape this URL and give me a list of prices' and Claude will handle it autonomously." },
    ...
    { category: "ai-agents", title: "Google ADK (Agent Development Kit)", provider: "Google", desc: "Google's open-source, code-first framework for building, evaluating, and deploying sophisticated AI agents — model-agnostic, multi-language (Python, TypeScript, Go, Java), and designed to make agent development feel like regular software development.", type: "GitHub", url: "https://github.com/google/adk-python",
      why: "Most agent frameworks force you into opinionated abstractions that break when your workflow gets complex. ADK applies real software development principles — modular components, evaluation pipelines, proper tooling — to agent creation. It's built by Google, battle-tested at scale, and works with any model, not just Gemini.",
      how: "Install with `pip install google-adk`, define your agent's tools and instructions in Python, then use ADK's built-in evaluation and deployment pipeline to ship it. Use it to build single agents, multi-agent teams, or RAG pipelines. The tutorials on The Unwind AI show concrete real-world patterns like sales intelligence agents and research agents built with ADK." },
    { category: "ai-agents", title: "Google ADK Samples", provider: "Google", desc: "Official collection of sample agents built with Google ADK — ready-to-run examples in Python, TypeScript, Go, and Java covering common patterns like multi-agent teams, tool use, and RAG pipelines.", type: "GitHub", url: "https://github.com/google/adk-samples" },
    ...
    // ── Rocket Templates ──
    { category: "rocket-templates", title: "Rocket.new Templates", provider: "Rocket.new", desc: "Production-ready app templates designed for AI-era projects — launch full-stack applications fast with pre-built patterns for common product use cases so you spend time on features, not scaffolding.", type: "Website", url: "https://www.rocket.new/templates",
      why: "Building the UI for an AI application from scratch takes too long and often results in generic designs. Rocket templates provide high-quality, pre-built components and layouts specifically optimized for AI tools (dashboards, chat interfaces, result grids). They are free to use and can save you days of frontend work.",
      how: "1. Visit rocket.new/templates. 2. Choose a template that matches your use case. 3. Copy the code or use the boilerplate as your starting point. Use these to quickly build the interface for your AI agents and tools." },
    ...
    { category: "ai-tools", title: "CTO.new", provider: "CTO.new", desc: "AI CTO assistant for technical decision-making — get architecture advice, technology stack recommendations, and engineering strategy guidance when you don't have a senior engineer to ask.", type: "Website", url: "https://cto.new/",
      why: "Most developers and founders hit a wall when making architecture decisions — do you build a monolith or microservices? PostgreSQL or MongoDB? REST or GraphQL? Getting these wrong costs weeks of rework. CTO.new gives you an experienced technical voice on demand, without the $300/hr consultant or the wait for a senior engineer to be available.",
      how: "Describe your project, your constraints, and the decision you're facing. CTO.new asks clarifying questions and returns concrete, opinionated recommendations with trade-offs explained. Use it before greenfield projects, when picking a tech stack, when evaluating a library, or when you need a second opinion on a system design. It saves you from expensive mistakes early." },
...
const categoryMeta = {
    ...
    "claude-code-skills": {
        icon: "\uD83E\uDDB8", label: "Claude Code Skills",
        desc: "Advanced custom skills and workflows to supercharge Claude Code performance and token efficiency",
        summary: "Why: Custom skills let you extend Claude Code's native capabilities with specialized tools, automation, and project-specific knowledge. They reduce token burn by up to 90% and ensure Claude always has the right context. How to use: Follow the step-by-step guides to install the CLI tools (NotebookLM, Playwright), then use the provided 'Option A' prompts to let Claude set up the skill structure for you. Add the corresponding rules to your CLAUDE.md to make them permanent."
    },
    "rocket-templates": {
        icon: "\uD83D\uDE80", label: "Rocket Templates",
        desc: "Production-ready UI and app templates to launch AI-era projects in minutes",
        summary: "Why: Building the UI for an AI app from scratch takes too long. Rocket templates provide curated, high-quality, and free-to-use boilerplate for common AI patterns. How to use: Browse the template library at rocket.new/templates, pick a UI that matches your project (dashboard, landing page, etc.), and use it as your starting point to skip the 'empty screen' phase of development."
    },
    "prompt-eng": {
